...
baseline_df = df_filtered[df_filtered['DisplayMethod'] == 'ILP'][['DatasetName', 'Budget', 'SumProb']]
...
ax1.set_ylabel("Expectation % Deviation from ILP")
ax1.set_xticks(sorted(df_dev['Budget'].unique()))
ax1.grid(True)
# ax1.legend(title="Method")
ax1.legend(title="Method", ncol=2, loc='lower left', bbox_to_anchor=(0.02, 0.4))
...
ax2.set_xlabel("Mean Expection of ILP", labelpad=6)
...
plt.grid(True)
plt.tight_layout()
plt.savefig("plots/Percentage_Deviation_from_ILP_.png", dpi=300)
plt.show()
...
# ax2.set_xticks(x_ticks[::2])
ax2.set_title("FOM % Deviation")
ax2.grid(True)

# --- Shared Legend ---
handles, labels = ax1.get_legend_handles_labels()
fig.legend(handles, labels, title="Method", loc="upper center", ncol=len(labels), frameon=False)

# Layout and save
plt.tight_layout(rect=[0, 0, 1, 0.93])  # leave space on top for legend
plt.savefig("plots/Combined_Runtime_Deviation_Clean.png", dpi=300, bbox_inches='tight')
plt.show()
