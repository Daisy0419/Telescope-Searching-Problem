...
# === Config ===
# folder_path = "large"
# folder_path = "large_with_Gurobi2"
folder_path = "large_with_gurobi_bound2"
...
df_filtered = df_filtered[(df_filtered['Budget'] > 0) & (df_filtered['Budget'] <= 1600)].copy()

df_normalized = df_filtered.groupby(["DatasetName", "Budget"]).apply(normalize_group).reset_index(drop=True)
df_normalized = df_normalized[(df_normalized['Budget'] > 0) & (df_normalized['Budget'] <= 1600)].copy()

# === Compute Deviation from GCP Baseline ===
baseline_df = df_filtered[df_filtered['DisplayMethod'] == 'GUROBI'][['DatasetName', 'Budget', 'SumProb']]
baseline_df = baseline_df.rename(columns={'SumProb': 'GUROBI_SumProb'})

df_with_baseline = df_filtered.merge(baseline_df, on=['DatasetName', 'Budget'])
df_with_baseline['DeviationFromGCP'] = df_with_baseline['SumProb'] - df_with_baseline['GUROBI_SumProb']
...
baseline = df_filtered[df_filtered['DisplayMethod'] == 'GUROBI'][['DatasetName', 'Budget', 'SumProb']]
...
plt.xlabel("Deadline (Seconds)")
plt.ylabel("FOM Deviation from GUROBI")
...
df_dev['FOMPctDeviation'] = 100 * (df_dev['SumProb'] - df_dev['BaselineFOM']) / df_dev['BaselineFOM']
agg_pct_dev = df_dev.groupby(['DisplayMethod', 'Budget'])['FOMPctDeviation'].agg(['mean', sem]).reset_index()
agg_gcp = df_dev[df_dev['DisplayMethod'] == 'GUROBI'].groupby('Budget')['BaselineFOM'].mean().reset_index()
...
ax1.set_ylabel("FOM % Deviation from GUROBI")
...
# Switch baseline from GCP to GUROBI
baseline = df_filtered[df_filtered['DisplayMethod'] == 'GUROBI'][['DatasetName', 'Budget', 'SumProb']]
baseline = baseline.rename(columns={'SumProb': 'BaselineFOM'})
...
plt.grid(True)
plt.tight_layout()
plt.savefig("plots/Absolute_Deviation_from_GUROBI.png", dpi=300)
plt.show()
...
plt.axhline(0, linestyle='--', color='black', linewidth=1)
plt.xlabel("Deadline (Seconds)")
plt.ylabel("% Deviation from GUROBI")
plt.legend(title="Method")
plt.grid(True)
plt.tight_layout()
plt.savefig("plots/Percentage_Deviation_from_GUROBI.png", dpi=300)
plt.show()
