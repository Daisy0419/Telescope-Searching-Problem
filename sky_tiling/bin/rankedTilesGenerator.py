...
# 		tileData = np.recfromtxt(tileFile, names=True)
	...
	# For the RTSS2025 paper, we added this new function
	# It is based on the plotTiles function below, but very simplified
	# It only generates an output table of tiles, but does not plot them
	def generateTable(self, ranked_tile_indices, allTiles_probs_sorted, CI, containment, max_rows = None):

		from utilities import AllSkyMap_basic

		# Compute cumulative probabilities of tiles, sorted in descending order of probability
		cumulative_probabilities = np.cumsum(allTiles_probs_sorted)

		# Just include those within the confidence interval CI
		include_tiles = cumulative_probabilities < CI

		# If we want to fully cover the CI, rather than staying strictly within  it,
		# then also mark the next tile for inclusion
		include_tiles[np.sum(include_tiles)] = containment

		# We can define a maximum number of rows to keep for small instances
		if max_rows is not None:
			include_tiles[max_rows:] = False

		# Filter
		ranked_tile_indices = ranked_tile_indices[include_tiles]		
		ranked_tile_probs = allTiles_probs_sorted[include_tiles]
		cumulative_probabilities = cumulative_probabilities[include_tiles]
		...
		output = np.vstack((ranks, ranked_tile_indices, RA_tile[ranked_tile_indices], Dec_tile[ranked_tile_indices], ranked_tile_probs, cumulative_probabilities)).T
		t = Table(rows=output, names=('Rank', 'index', 'RA', 'Dec', 'Probability', 'CumulativeProb'), dtype=('i4', 'i4', 'f8', 'f8', 'f8', 'f8'))
		return t 
	...
	def plotTiles(self, ranked_tile_indices, allTiles_probs_sorted, tileFile=None, FOV=None,
		...
		if title:
			pl.title(title)
		m = AllSkyMap_basic.AllSkyMap(projection='moll')
...
# 		tileData = np.recfromtxt(tileFile, names=True)
		...
		edges = np.empty((ranked_tile_indices.size, 14))
		print "Num tiles to save:", ranked_tile_indices.size
		print edges.shape

		for i, ii in enumerate(ranked_tile_indices):
			...
			if tileEdges:
				...
				edges[i,:] = [RAP1, RAP2, RAP3, RAP4, DecP1, DecP2, DecP3, DecP4, \
				   ra_up_left, ra_up_right, ra_down_left, ra_down_right, dec_up, dec_down]
				

			else:
				m.plot(RAP_peak, DecP_peak, 'ko', markersize=5*lw, mew=1, alpha=alpha)
		np.save("tile_edges.npy", edges)
