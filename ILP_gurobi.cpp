...
std::vector<int> solveWithGurobi(const ProblemData& data, double mipGap, double timeLimit,
    ...
    try {
        ...
        //     // Set all other x[i][j] and is_end[i] to 0 explicitly (optional but recommended)
        //     for (int i = 0; i < N; i++) {
        //         for (int j = 0; j < N; j++) {
        //             if (x[i][j].get(GRB_DoubleAttr_Start) != 1.0) {
        //                 x[i][j].set(GRB_DoubleAttr_Start, 0.0);
        //             }
        //         }
        //         if (i != endNode && is_end[i].get(GRB_DoubleAttr_Start) != 1.0) {
        //             is_end[i].set(GRB_DoubleAttr_Start, 0.0);
        //         }
        //     }
