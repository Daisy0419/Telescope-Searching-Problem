...
// std::vector<int> generate_random_st_path(const std::vector<std::vector<double>>& costs, int current_city, int end_city, double budget) {
//     std::vector<int> visited{current_city};
//     int num_cities = costs.size();
//     std::vector<int> unvisited(num_cities);
//     std::iota(unvisited.begin(), unvisited.end(), 0);
//     unvisited.erase(std::remove(unvisited.begin(), unvisited.end(), current_city), unvisited.end());
...
//     for (int next_city : unvisited) {
//         if (costs[current_city][next_city] + costs[next_city][end_city] <= budget) {
//             visited.push_back(next_city);
//             budget -= costs[current_city][next_city];
//             current_city = next_city;
//         }
...
//     if (costs[current_city][end_city] <= budget) {
//         visited.push_back(end_city);
//     } else {
//         visited = {visited.front(), end_city};  // fallback
//     }
//     return visited;
// }

// std::vector<std::vector<int>> random_st_paths(const std::vector<std::vector<double>>& costs, int current_city, int end_city, double budget, int num_paths) {
//     std::vector<std::vector<int>> paths(num_paths);
...
// std::vector<int> greedy_path_st(const std::vector<std::vector<double>>& costs, int start_city, int end_city, double budget) {
//     std::vector<int> visited{start_city};
//     int num_cities = costs.size();
//     int current_city = start_city;
...
//         visited.push_back(end_city);
//     else
//         visited = {start_city, end_city};

//     return visited;
// }

// std::vector<std::vector<int>> partial_greedy_paths(const std::vector<std::vector<double>>& costs, int current_city, int end_city, double budget, int num_paths) {
//     std::vector<std::vector<int>> paths;
//     for (int i = 0; i < num_paths; ++i) {
//         std::vector<int> path = generate_random_st_path(costs, current_city, end_city, budget);
//         if (path.size() < 3) {
...
//         std::vector<int> head(path.begin(), path.begin() + segment1 + 1);
//         std::vector<int> tail(path.begin() + segment2, path.end());

//         double cost = calculate_path_cost(head, costs) + calculate_path_cost(tail, costs);
//         double remaining_budget = budget - cost;

//         std::vector<int> middle = greedy_path_st(costs, path[segment1], path[segment2], remaining_budget);
//         if (middle.size() > 2)
//             head.insert(head.end(), middle.begin() + 1, middle.end() - 1);
//         head.insert(head.end(), tail.begin(), tail.end());
...
// void repair_st_path(const std::vector<std::vector<double>>& costs, const std::vector<double>& prizes, std::vector<int>& path, double budget) {
//     if (path.size() < 2) return;
...
//         total_cost -= costs[u][v] + costs[v][w] - costs[u][w];
//         path.erase(path.begin() + least_prize_index);
//     }

//     int current_city = path[path.size() - 2];
//     std::vector<bool> visited(costs.size(), false);
//     for (int city : path) visited[city] = true;
...
//     offspring[size - 1] = parent1[size - 1];

//     auto segment = unique_random_ints(1, size - 2, 2);
//     int start = std::min(segment[0], segment[1]);
//     int end = std::max(segment[0], segment[1]);

//     for (int i = start; i <= end; ++i) offspring[i] = parent1[i];
...
//         if (idx >= size - 1) break;
//         int city = parent2[i];
//         if (std::find(offspring.begin() + 1, offspring.end() - 1, city) == offspring.end() - 1)
//             offspring[idx++] = city;
//     }
//     return offspring;
// }

// int find_next_city(int current_city, int end_city, const std::vector<std::vector<double>>& costs, const std::vector<double>& prizes,
//                    const std::unordered_set<int>& unvisited, double budget_remaining) {
//     int next_city = -1;
//     double best_score = -1e9;
//     for (int city : unvisited) {
//         double cost_to = costs[current_city][city] + costs[city][end_city];
//         if (cost_to <= budget_remaining) {
//             double score = prizes[city] / (costs[current_city][city] + 1e-6);
//             if (score > best_score) {
//                 best_score = score;
//                 next_city = city;
//             }
...
// std::vector<int> heuristic_crossover_st(const std::vector<int>& parent1, const std::vector<int>& parent2,
//                                         const std::vector<std::vector<double>>& costs, const std::vector<double>& prizes,
//                                         double budget) {
//     int s = parent1.front(), t = parent1.back();
//     std::vector<int> offspring{ s };
//     std::unordered_set<int> unvisited(parent1.begin() + 1, parent1.end() - 1);
//     unvisited.insert(parent2.begin() + 1, parent2.end() - 1);
...
//     while (!unvisited.empty()) {
//         double cost_to_t = costs[current][t];
//         int next = find_next_city(current, t, costs, prizes, unvisited, remaining - cost_to_t);
//         if (next == -1) break;
//         double step_cost = costs[current][next];
...
//     for (int i = 0; i < num_swap_pairs; ++i) {
//         auto idx = unique_random_ints(1, path.size() - 2, 2);
//         std::swap(path[idx[0]], path[idx[1]]);
//     }
//     return path;
// }

// std::vector<int> insert_mutate_st(std::vector<int> path, int num_insert) {
//     if (path.size() < 3) return path;
//     for (int i = 0; i < num_insert; ++i) {
//         auto idx = unique_random_ints(1, path.size() - 2, 2);
//         int city = path[idx[0]];
//         path.erase(path.begin() + idx[0]);
//         path.insert(path.begin() + idx[1], city);
//     }
//     return path;
// }
...
//     if (path.size() < 3) return path;
//     auto idx = unique_random_ints(1, path.size() - 2, 2);
//     std::reverse(path.begin() + std::min(idx[0], idx[1]), path.begin() + std::max(idx[0], idx[1]) + 1);
//     return path;
// }
...
// //     std::sort(paths_copy.begin(), paths_copy.end(), [&](const auto& a, const auto& b) {
// //         double a_score = 0, b_score = 0;
// //         for (int c : a) a_score += prizes[c];
// //         for (int c : b) b_score += prizes[c];
// //         return a_score > b_score;
// //     });
// //     return std::vector<std::vector<int>>(paths_copy.begin(), paths_copy.begin() + std::min((int)paths_copy.size(), top_n));
// // }

// // std::vector<int> get_best_path(const std::vector<std::vector<int>>& paths, const std::vector<double>& prizes) {
// //     return *std::max_element(paths.begin(), paths.end(), [&](const auto& a, const auto& b) {
// //         double a_score = 0, b_score = 0;
// //         for (int c : a) a_score += prizes[c];
// //         for (int c : b) b_score += prizes[c];
// //         return a_score < b_score;
...
// //     path.erase(std::remove_if(path.begin(), path.end(), [&](int city) {
// //         return !seen.insert(city).second;
// //     }), path.end());
// // }

// // ------------------- Main Genetic Loop -------------------------

// std::vector<int> evolution_st(const std::vector<std::vector<double>>& costs, const std::vector<double>& prizes, int start_city, int end_city,
//                               int num_path, double budget, int evolution_itr, std::vector<std::vector<int>>& init_paths) {
//     int num_random_path = num_path - init_paths.size();
//     std::vector<std::vector<int>> paths = random_st_paths(costs, start_city, end_city, budget, num_random_path);
//     paths.insert(paths.end(), init_paths.begin(), init_paths.end());
//     int num_mutation = 0.5 * num_path;
//     int num_cross = 0.5 * num_path;
//     int top_n = num_path * 0.94;
//     int num_new_gen = num_path - top_n;

//     for (int i = 0; i < evolution_itr; ++i) {
//         std::vector<std::vector<int>> mutate_paths(num_mutation);
//         #pragma omp parallel for
//         for (int j = 0; j < num_mutation; ++j) {
//             int idx = random_int(0, paths.size() - 1);
//             auto mutated = j < num_mutation / 3
//                          ? swap_mutate_st(paths[idx], 5)
//                          : (j < 2 * num_mutation / 3
//                              ? insert_mutate_st(paths[idx], 5)
//                              : reverse_mutate_st(paths[idx]));
//             repair_st_path(costs, prizes, mutated, budget);
//             mutate_paths[j] = std::move(mutated);
//         }

//         std::vector<std::vector<int>> crossover_paths(num_cross);
//         #pragma omp parallel for
//         for (int j = 0; j < num_cross; ++j) {
//             auto idx = unique_random_ints(0, paths.size() - 1, 2);
//             auto& p1 = paths[idx[0]], &p2 = paths[idx[1]];
//             auto child = j < num_cross / 2
//                        ? ordered_crossover_st(p1, p2)
//                        : heuristic_crossover_st(p1, p2, costs, prizes, budget);
//             repair_st_path(costs, prizes, child, budget);
//             crossover_paths[j] = std::move(child);
//         }

//         std::vector<std::vector<int>> all_paths = paths;
//         all_paths.insert(all_paths.end(), mutate_paths.begin(), mutate_paths.end());
//         all_paths.insert(all_paths.end(), crossover_paths.begin(), crossover_paths.end());

//         paths = select_top_paths(all_paths, costs, prizes, top_n);
//         auto new_gen = random_st_paths(costs, start_city, end_city, budget, num_new_gen);
//         paths.insert(paths.end(), new_gen.begin(), new_gen.end());
//     }

//     return get_best_path(paths, prizes);
// }

// std::vector<int> genetic_optimization_st(const std::vector<std::vector<double>>& costs, const std::vector<double>& prizes, double budget,
//                                          int start_city, int end_city, std::vector<std::vector<int>> init_paths) {
//     return evolution_st(costs, prizes, start_city, end_city, 100, budget, 1000, init_paths);
// }